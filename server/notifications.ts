...
/**
 * Notify all managers when a promoter registers entry or exit.
 */
export async function notifyClockEntry(
  promoterName: string,
...
/**
 * Notify a promoter when their photo is approved.
 */
export async function notifyPhotoApproved(
  ...
  await sendPushNotifications(
    tokens.map((to) => ({
      ...
      title: "✅ Foto aprovada!",
      body: managerNotes
        ? `Sua foto de ${brandName} foi aprovada. Obs: ${managerNotes}`
        : `Sua foto de ${brandName} foi aprovada pelo gestor.`,
      data: { type: "photo_approved", action: "approved" },
      sound: "default" as const,
      priority: "high" as const,
    }))
  );
...
/**
 * Notify a promoter when their photo is rejected.
 */
export async function notifyPhotoRejected(
  ...
  await sendPushNotifications(
    tokens.map((to) => ({
      ...
      title: "❌ Foto recusada",
      body: managerNotes
        ? `Sua foto de ${brandName} foi recusada. Motivo: ${managerNotes}`
        : `Sua foto de ${brandName} foi recusada pelo gestor.`,
      data: { type: "photo_rejected", action: "rejected" },
      sound: "default" as const,
      priority: "high" as const,
    }))
  );
