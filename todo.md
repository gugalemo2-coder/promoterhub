...
## Fase 25 — Correção de permissões da conta Master

- [x] Corrigir isManager no index.tsx para incluir master (appRole === "manager" || appRole === "master")
- [x] Auditar todos os arquivos de tela que usam isManager e corrigir onde necessário (clock, files, materials, photos, alerts, reports, stores, team)
- [x] Corrigir schema do banco de dados (promoter_profiles.appRole enum incluir master)
- [x] Corrigir routers.ts setRole para aceitar master
- [x] Corrigir custom-auth.ts para aceitar master na atribuição de role
- [x] Executar migração do banco de dados (0007_smiling_juggernaut.sql)
