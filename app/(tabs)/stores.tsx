...
export default function StoresScreen() {
  ...
  // Fix: use user.id (not user.userId) — getAllPromoterUsersWithProfile returns { id, name, login }
  const getPromoterName = (promoterId: number | null | undefined) => {
    if (!promoterId || !promoterUsers) return null;
    const user = (promoterUsers as any[]).find((u) => u.id === promoterId);
            ...
            <View style={[styles.promoterSelector, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              {!promoterUsers || promoterUsers.length === 0 ? (
                ...
                <>
                  ...
                  >
                    <View style={[styles.promoterAvatar, { backgroundColor: colors.border }]}>
                    ...
                    {form.promoterId === null && <Ionicons name="checkmark-circle" size={20} color={colors.primary} />}
                  </Pressable>

                  {(promoterUsers as any[]).map((user) => {
                    const isSelected = form.promoterId === user.id;
                    return (
                      <Pressable
                        key={user.id}
                        style={({ pressed }) => [
                        ...
                        onPress={() => setForm((f) => ({ ...f, promoterId: user.id }))}
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          <Text style={[styles.promoterOptionText, { color: isSelected ? colors.primary : colors.foreground }]}>
                            {user.name ?? "Sem nome"}
                          </Text>
                          {user.login && (
                            <Text style={[styles.promoterOptionEmail, { color: colors.muted }]} numberOfLines={1}>
                              {user.login}
                            </Text>
                          )}
                        </View>
...
const styles = StyleSheet.create({
  ...
  flex1: { flex: 1 },
  flex2: { flex: 2 },
  promoterSelector: { borderRadius: 14, borderWidth: 1, overflow: "hidden", marginTop: 4 },
  promoterEmpty: { flexDirection: "row", alignItems: "center", gap: 10, padding: 16 },
  promoterEmptyText: { fontSize: 14 },
  promoterOption: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 14, paddingVertical: 12 },
